...
class Config(commands.Cog):
    ...
    async def cog_app_command_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        ...
        if isinstance(error, app_commands.MissingPermissions):
            ...
            print(f"An error occurred in a config command: {error}")
            await interaction.response.send_message(
            ...
            if role in member.roles:
                ...
                if member.nick != expected_nickname:
                    try:
                        ...
                        failed_count += 1
                        print(f"Failed to update {member.name} during run-rule: {e}")
                else:
    ...
    async def _sync_all_guilds_history(self):
        ...
        print("Starting baseline nickname history sync...")
        synced_guilds = 0
        for guild in self.bot.guilds:
            try:
                print(f"Syncing history for guild: {guild.name} ({guild.id})")
                
                all_rules = await db.get_all_rules(guild.id)
                if not all_rules:
                    print(f" -> No rules found for {guild.name}, skipping.")
                    continue
                ...
                print(f" -> Scanned {member_count} members, saved/updated {history_entries_saved} history entries.")
                synced_guilds += 1

            except discord.Forbidden:
                print(f" -> ERROR: Missing permissions to fetch members in {guild.name}. Skipping.")
            except Exception as e:
                print(f" -> ERROR: An unexpected error during history sync for {guild.name}: {e}")

        print(f"--- Baseline sync complete. Processed {synced_guilds}/{len(self.bot.guilds)} guilds. ---")
